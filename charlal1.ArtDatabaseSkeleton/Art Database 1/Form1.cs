...
namespace Art_Database_1
{
    public partial class Form1 : Form
    {
        ...
        {
            displayQuery(artists.Where(a => a.Country == "Italy"));                        
        }

        private void displayQuery(IEnumerable<Artist> result) 
        {
            listBox1.Items.Clear();
        ...
        private void displayQuery(IEnumerable<Painting> result)
        {
            listBox1.Items.Clear();
        ...
        {
            displayQuery(paintings.Where(p => p.Year < 1800).OrderBy(p => p.Year));            
        }
        ...
        private void btnOldest_Click(object sender, EventArgs e)
        {
            ...
            listBox1.Items.Add(paintings.OrderBy(p => p.Year).First());
        }
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            ...
            IEnumerable<Painting> result;
            
            result = paintings.Where(p => String.Equals(p.Artist, textBox1.Text));
        ...
        private void btnNbyCountry_Click(object sender, EventArgs e)
        {
            ...
            var result = from artist in artists
                         ...
                         group artist by artist.Country into grouping
                         orderby grouping.Count()
                         select new { country = grouping.Key, count = grouping.Count() };

            foreach (var item in result)
            {
                listBox1.Items.Add(item.count + " " + ((item.count > 1) ? "paintings" : "painting") + " from " + item.country);
            }            
        }
        ...
        private void button7_Click(object sender, EventArgs e)
        {
            ...
            foreach (var item in result)
            {
                listBox1.Items.Add(item.LastName + "\t\t" + item.Year + "\t\t" + item.Method + "\t\t" + item.Title);
            }          
        }
